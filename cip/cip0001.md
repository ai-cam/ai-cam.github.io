...
1. **Audit existing content** — script to compare `slug` field against `title | slugify` across all collections, output list of mismatches.
2. **Add redirects for every mismatch** — for each item where the existing `slug` differs from `title | slugify`, add a `redirect_from` entry in the frontmatter pointing from the title-derived URL to the existing slug URL. This step is a hard gate: no content files may be modified in step 4 until every mismatch has a redirect. This is required by REQ-0002.
3. **Add `jekyll-redirect-from`** to Gemfile if not already present; document the redirect procedure for future title changes.
4. **Update `admin/config.yml`** — remove `slug` fields from all folder collections, change collection-level slug templates, update `identifier_field`.
5. **Update Jekyll templates** — find all references to `page.slug` and update as needed.
6. **Test build** — confirm no URL conflicts, no broken internal links, no missing redirects.
7. **Deploy and monitor** — watch Netlify build logs and search console for unexpected 404s in the two weeks following deployment.
...
- [ ] Audit script: compare slug vs title | slugify across all collections
- [ ] Redirects added for every slug/title mismatch (hard gate — REQ-0002)
- [ ] jekyll-redirect-from confirmed in Gemfile
- [ ] Update admin/config.yml
- [ ] Update Jekyll templates referencing page.slug
