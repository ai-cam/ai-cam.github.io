...
2. **Update `admin/config.yml`** — remove `slug` fields from all folder collections, change collection-level slug templates, update `identifier_field`.
3. **Update Jekyll templates** — find all references to `page.slug` and update as needed.
4. **Add `jekyll-redirect-from`** to Gemfile if not present; document redirect procedure.
5. **Test build** — confirm no URL conflicts, no broken internal links.
6. **Deploy and monitor** — watch Netlify build logs and search console for unexpected 404s.
...
- [ ] Confirm jekyll-redirect-from in Gemfile
- [ ] Build passes with zero conflict warnings
- [ ] Post-deploy URL spot-check
